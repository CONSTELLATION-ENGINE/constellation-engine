...
### Fixed
- **Windows release artifacts now match updater manifest names** (`scripts/build-platform.sh`): the cross-platform build script normalizes NSIS output from `Constellation Setup <version>.exe` to `Constellation-Setup-<version>.exe`, matching `latest.yml` and website download-link expectations.
